...
if Modules == nil then
	...
	function StdModule.say(cid, message, keywords, parameters, node)
		...
		if cost and cost > 0 and Player(cid):getLevel() >= 50 then
			if parameters.discount then
				cost = cost - StdModule.travelDiscount(Player(cid), parameters.discount)
			end
		...
		local parseInfo = {[TAG_PLAYERNAME] = Player(cid):getName(), [TAG_TRAVELCOST] = costMessage }
