...
local function addTravelKeyword(keyword, cost, destination, text, condition)
	...
	local travelKeyword = keywordHandler:addKeyword({keyword}, StdModule.say, {npcHandler = npcHandler, text = text or 'Do you seek a passage to ' .. string.titleCase(keyword) .. ' for |TRAVELCOST|?', cost = cost, discount = 'postman'})
